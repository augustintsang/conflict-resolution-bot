...
def generate_objective():
    ...
    conversation = """
    Steve: Alright, so we all agree on the idea—an AI-powered restaurant reservation agent. It checks availability, 
    books a table, even suggests options based on user preferences.
    Ashwini: Yeah, and it should be able to call or message restaurants that don’t have an online system.
    Jose: Love it. So for the agent framework, LlamaIndex seems like a no-brainer. It’s one of the sponsors, and it’s 
    solid for retrieval-augmented generation.
    Ashwini: Agreed. Plus, we get points for using sponsor tools. Let’s lock that in.
    (They nod, moving on to the next decision.)
    Steve: Now, the LLM. Gemini’s a sponsor, so we’d benefit from using it. But OpenAI o1 is... well, OpenAI o1. 
    It’s top-tier.
    Jose: Yeah, but we’re optimizing for both performance and hackathon perks. Gemini’s not bad—it gives decent results, 
    and it keeps us in good standing with the event judges.
    Ashwini: True, but we’re not required to use sponsor tools. If OpenAI’s going to get us better responses, 
    ...
    Jose: Okay, what about connecting to the web? We need real-time data—restaurant availability, location details, 
    even user reviews.
    Ashwini: AgentQL is a sponsor, so it’d be cheaper for us. But I have no clue how tricky it is to implement.
    Steve: Yeah, I haven’t seen many people using it yet. On the other hand, Perplexity API is reliable. 
    More expensive, but we know it works well.
    Jose: So do we go for ease-of-use and proven reliability with Perplexity? Or do we save money and earn sponsor points 
    with AgentQL?
    Ashwini: That’s the problem. I don’t know how long AgentQL would take to set up. If it’s a pain, 
    we could waste a lot of time.
    Steve: And if we pick OpenAI o1 for the LLM, plus Perplexity for web search, that’s two major non-sponsor choices. 
    Could hurt us.
    ...
    Steve: Yeah, and we need to decide if Gemini is “good enough” or if OpenAI o1 is worth breaking from 
    the sponsor incentives.
    ...
    #    Make sure your environment has GEMINI_API_KEY set, or replace this with your actual key.
    ...
    # 6) Return the raw JSON, but parse it first so FastAPI can enforce the schema.
